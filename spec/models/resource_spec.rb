...
RSpec.describe Gaas::Resource, type: :model do
  valid_path = File.join Gaas::ROOT_PATH, 'spec', 'assets', 'valid_app'
  missing_path = File.join Gaas.config.test_tmp_dir, 'missing_app'
  ...
  context 'is not valid' do
    ...
    it 'without a resource_type' do
      record = described_class.new name: 'Test Resource', path: valid_path
      expect(record).to_not be_a_valid_record
      expect(record.errors.full_messages).to eq ['Resource type can\'t be blank']
    end

    it 'without a path' do
      record = described_class.new name: 'Test Resource', resource_type: 'local'
      expect(record).to_not be_a_valid_record
    ...
    it 'with an non-existing path' do
      record = described_class.new name: 'Test Resource', resource_type: 'local', path: missing_path
      expect(record).to_not be_a_valid_record
    ...
    it 'if the same name is used twice' do
      described_class.create name: 'Test Resource', resource_type: 'local', path: valid_path
      record = described_class.new name: 'Test Resource', resource_type: 'local', path: valid_path
      expect(record).to_not be_a_valid_record
    ...
    it 'if the build_url is not a url' do
      record = described_class.new name: 'Test Resource', resource_type: 'local', path: valid_path, build_url: 'test.ch/asdf'
      expect(record).to_not be_a_valid_record
      expect(record.errors.full_messages).to eq ['Build url must be a valid URL']
    end

    it 'if the build_image_url is not a url' do
      record = described_class.new name: 'Test Resource', resource_type: 'local', path: valid_path, build_image_url: 'test.ch/asdf'
      expect(record).to_not be_a_valid_record
  ...
  it 'generates the correct resource_type data' do
    ...
    expect(described_class.resource_types.keys).to eq Gaas::GemsuranceService.fetchers.keys
