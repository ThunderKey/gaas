...
RSpec.describe Gaas::GemsuranceService, type: :service do
  it 'includes valid fetchers' do
    expect(described_class.fetchers.keys).to eq ['local']
    described_class.fetchers.each do |name, fetcher|
  ...
  it 'loads the gems correctly' do
    ...
    FileUtils.cp File.join(Gaas::ROOT_PATH, 'spec', 'assets', 'simple_gemsurance_report.yml'), report_file
  ...
  it 'loads the gems with vulnerabilities correctly' do
    ...
    report_file = service.gemsurance_yaml_file
    FileUtils.mkdir_p service.dirname
    FileUtils.cp File.join(Gaas::ROOT_PATH, 'spec', 'assets', 'vulnerable_gemsurance_report.yml'), report_file
