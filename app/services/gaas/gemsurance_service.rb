class Gaas::GemsuranceService < ApplicationService
  @@fetchers = {
    'local' => LocalFetcher,
  }
  ...
  def dirname
    @dirname ||= begin
      ...
      File.join Gaas.config.private_dir, 'gemsurance_reports', resource_id
    end
  end
  ...
  def load_gems
    ...
    gems.each do |name, gem_data|
      info = Gaas::GemInfo.where(name: name).first_or_initialize
      ...
      info.save!
      Gaas::GemVersion.where(gem_info: info, version: gem_data['newest_version']).first_or_create! if gem_data['newset_version'] != gem_data['bundle_version']
      version = Gaas::GemVersion.where(gem_info: info, version: gem_data['bundle_version']).first_or_create!
